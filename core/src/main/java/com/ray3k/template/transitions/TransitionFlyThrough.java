...
public class TransitionFlyThrough implements Transition {
    ...
    public void draw(Batch batch, float delta) {
        ...
        float width = Gdx.graphics.getWidth() * (interpolation.apply((te.time + delta) / te.duration) * (scale - 1) + 1);
        float height = Gdx.graphics.getHeight() * (interpolation.apply((te.time + delta) / te.duration) * (scale - 1) + 1);
