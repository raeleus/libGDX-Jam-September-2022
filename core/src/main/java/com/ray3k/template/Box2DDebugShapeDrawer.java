...
public class Box2DDebugShapeDrawer {
    ...
    private void drawSolidCircle (Vector2 center, float radius, Vector2 axis, Color color) {
        ...
        for (int i = 0; i < 20; i++, angle += angleInc) {
            ...
            shapeDrawer.line(m2p(lv.x), m2p(lv.y), m2p(v.x), m2p(v.y));
            lv.set(v);
        }
        shapeDrawer.line(m2p(f.x), m2p(f.y), m2p(lv.x), m2p(lv.y));
        shapeDrawer.line(m2p(center.x), m2p(center.y), m2p(center.x + axis.x * radius), m2p(center.y + axis.y * radius));
    }
    ...
    private void drawSolidPolygon (Vector2[] vertices, int vertexCount, Color color, boolean closed) {
        ...
        if (closed) shapeDrawer.line(m2p(f.x), m2p(f.y), m2p(lv.x), m2p(lv.y));
    }
    ...
    private void drawSegment (Vector2 x1, Vector2 x2, Color color) {
        ...
        shapeDrawer.line(m2p(x1.x), m2p(x1.y), m2p(x2.x), m2p(x2.y));
    }
    ...
    private void drawContact (Contact contact) {
        ...
        shapeDrawer.circle(m2p(point.x), m2p(point.y), 2);
    }
